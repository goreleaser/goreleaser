...
func asHeadString(r Repo) string {
	return strings.Join(removeEmpty(r.Owner, r.Name, r.Branch), ":")
}
...
func (c *githubClient) OpenPullRequest(
	...
	if base.Owner == "" {
		base.Owner = head.Owner
	}
	if head.Branch == "" {
		head.Branch = base.Branch
	}
	log := log.
		WithField("base", asHeadString(base)).
		WithField("head", asHeadString(head))
	...
	pr, res, err := c.client.PullRequests.Create(ctx, base.Owner, base.Name, &github.NewPullRequest{
		...
		Head:  github.String(asHeadString(head)),
