...
func TestGitlabCreateBranchExists(t *testing.T) {
	totalRequests := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		totalRequests++
		defer r.Body.Close()
	...
	_, err = client.CreateBranch(ctx, repo)
	require.NoError(t, err)
	require.Equal(t, 2, totalRequests)
}

func TestGitlabCreateBranchNotExists(t *testing.T) {
	totalRequests := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		totalRequests++
		defer r.Body.Close()
		...
		if r.Method == "POST" {
			...
			// Assume the normal GET is gonna be not found/404
	...
	created, err := client.CreateBranch(ctx, repo)
