...
// responsibility — typically done in the `before:` hook with `npm ci`
// or `pnpm install --frozen-lockfile`. This function deliberately
// does not run any install step itself: the auto-bundle has to be
// safe to invoke unconditionally, so it sticks to the one command we
// can guarantee will not mutate the project's lockfile or touch the
// network (`npm run build` against an existing `node_modules/`).
//
...
func runNPMBuild(ctx context.Context, dir string, env []string, stdout, stderr io.Writer) error {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("node: npm run build: %w", err)
	}
	return nil
}
