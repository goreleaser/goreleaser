...
func (t *Template) Apply(s string) (string, error) {
	...
	tmpl, err := template.New("tmpl").
		...
		Funcs(template.FuncMap{
			...
			"dir":           func(s string) string { return filepath.ToSlash(filepath.Dir(s)) },
			"base":          func(s string) string { return filepath.ToSlash(filepath.Base(s)) },
			"abs":           func(s string) (string, error) { r, err := filepath.Abs(s); return filepath.ToSlash(r), err },
			"incmajor":      incMajor,
			"incminor":      incMinor,
			"incpatch":      incPatch,
			"filter":        filter(false),
