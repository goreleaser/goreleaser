...
var (
	...
	ErrMultipleArchivesSameArch = errors.New("one aur can handle only one archive of each architecture. Consider using ids in the aurs section")
)
...
func doPublish(ctx *context.Context, pkgs []*artifact.Artifact) error {
	...
	summary.Appendf("Pushed AUR package `%s` to `%s`", cfg.Name, cfg.GitURL)
	return nil
}
