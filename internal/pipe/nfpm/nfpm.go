...
func create(ctx *context.Context, fpm config.NFPM, format string, artifacts []*artifact.Artifact) error {
	...
	infoArch := artifacts[0].Goarch + artifacts[0].Go386 + artifacts[0].Goarm + artifacts[0].Gomips + artifacts[0].Goppc64 + artifacts[0].Goriscv64 // key used for the ConventionalFileName et al
