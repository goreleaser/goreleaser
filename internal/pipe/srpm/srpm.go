...
import (
	...
	_ "github.com/goreleaser/nfpm/v2/srpm" // blank import to register the format
)
...
func (Pipe) Run(ctx *context.Context) error {
	...
	// Get the spec template. If srpm.SpecTemplateFile is set, read it from that
	// file, otherwise use the value of srpm.SpecTemplate.
	var specTemplate string
	if srpm.SpecTemplateFile != "" {
		specTemplateBytes, err := os.ReadFile(srpm.SpecTemplateFile)
		if err != nil {
			return err
		}
		specTemplate = string(specTemplateBytes)
	} else {
		specTemplate = srpm.SpecTemplate
	}
	...
	specContents, err := t.Apply(specTemplate)
	if err != nil {
		return err
	}
	...
	contents = append(contents, &files.Content{
		...
		Type:        "", // FIXME @caarlos0 is this correct?
		Packager:    srpm.Packager,
