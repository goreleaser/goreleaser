...
func doPublish(ctx *context.Context, cask *artifact.Artifact, cl client.Client) error {
	...
	if brew.Repository.Git.URL != "" {
		...
		summary.Appendf("Updated homebrew cask `%s` in `%s`", cask.Name, cmp.Or(repo.String(), brew.Repository.Git.URL))
		return nil
	}
	...
	if !brew.Repository.PullRequest.Enabled {
		...
		summary.Appendf("Updated homebrew cask `%s` in `%s`", cask.Name, repo.String())
		return nil
	}
