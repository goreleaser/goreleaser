...
func assertDefaultTemplateData(t *testing.T, formulae string) {
	t.Helper()
	require.Contains(t, formulae, "cask Test do")
...
func TestFullPipe(t *testing.T) {
	...
	for name, tt := range map[string]testcase{
		...
		t.Run(name, func(t *testing.T) {
			...
			distFile := filepath.Join(folder, "casks", "Casks", name+".rb")
			...
			require.Equal(t, distFile, casks[0].Path)
...
func TestRunPipeNameTemplate(t *testing.T) {
	...
	require.NoError(t, f.Close())
	client := client.NewMock()
	distFile := filepath.Join(folder, "casks", "foo_is_bar.rb")
...
func TestRunPipeMultipleCasksWithSkip(t *testing.T) {
	folder := t.TempDir()
	ctx := testctx.NewWithCfg(
		config.Project{
			Dist:        folder,
	...
	require.EqualError(t, publishAll(ctx, cli), `casks.skip_upload is set`)
	require.True(t, cli.CreatedFile)

	for _, brew := range ctx.Config.Casks {
		distFile := filepath.Join(folder, "casks", brew.Name+".rb")
...
func TestRunPipeForMultipleAmd64Versions(t *testing.T) {
	for name, fn := range map[string]func(ctx *context.Context){
		...
		t.Run(name, func(t *testing.T) {
			...
			client := client.NewMock()
			distFile := filepath.Join(folder, "casks", name+".rb")
...
func TestRunPipeUniversalBinary(t *testing.T) {
	...
	require.NoError(t, f.Close())
	client := client.NewMock()
	distFile := filepath.Join(folder, "casks", "unibin.rb")
