...
builds:
  ...
  -
    ...
    # This field is templateable.
    # Envs with empty keys or values are ignored.
    #
    # Default is empty.
    env:
      # simple envs:
...
This example creates once `CC_` and `CCX_` variable for each platform, and then set `CC` and `CCX` to the right one:
